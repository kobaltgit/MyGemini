...
from config.settings import (
    ADMIN_USER_ID, CALLBACK_ADMIN_MAIN_MENU, CALLBACK_ADMIN_TOGGLE_MAINTENANCE,
...
async def handle_broadcast_cancel(call: types.CallbackQuery, bot: AsyncTeleBot):
    ...
    fake_message = types.Message(message_id=call.message.message_id, from_user=call.from_user, date=0, chat=call.message.chat, content_type='text', options={}, json_string="")
...
async def handle_stats_menu(call: types.CallbackQuery, bot: AsyncTeleBot):
    """Показывает меню глобальной статистики."""
    user_id = call.from_user.id
    lang_code = await db_manager.get_user_language(user_id)
...
async def _get_user_info_text(user_id_to_check: int, lang_code: str) -> str:
...
@admin_required
async def handle_user_id_for_management(message: types.Message, bot: AsyncTeleBot):
    """Получает ID, ищет пользователя и выводит его карточку."""
    admin_id = message.from_user.id
    lang_code = await db_manager.get_user_language(admin_id)
    ...
    user_info_text = await _get_user_info_text(user_id_to_manage, lang_code)
...
async def handle_toggle_block_user(call: types.CallbackQuery, bot: AsyncTeleBot):
    ...
    # Обновляем карточку
    new_user_info_text = await _get_user_info_text(user_id_to_toggle, lang_code)
...
async def handle_reset_api_key(call: types.CallbackQuery, bot: AsyncTeleBot):
    """Сбрасывает API-ключ пользователя."""
    admin_id = call.from_user.id
    lang_code = await db_manager.get_user_language(admin_id)
...
def register_admin_handlers(bot: AsyncTeleBot):
    ...
    bot.register_message_handler(
        handle_user_id_for_management,
        state=STATE_ADMIN_WAITING_FOR_USER_ID_TO_MANAGE,
        pass_bot=True
    )
    bot.register_message_handler(
        handle_cancel_state,
