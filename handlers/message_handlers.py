...
async def handle_any_message(message: types.Message, bot: AsyncTeleBot):
    ...
    try:
        # Сначала проверяем, находится ли пользователь в каком-либо состоянии
        if current_state:
            await handle_stateful_message(bot, message, current_state)
        # Если нет состояния, обрабатываем как обычный запрос
...
async def handle_stateful_message(bot: AsyncTeleBot, message: types.Message, current_state: str):
    ...
    # Если пользователь в состоянии, но прислал не текст, а что-то другое
    ...
    if handler:
        ...
        # На случай, если состояние есть, а обработчика нет
...
async def handle_state_translate(bot: AsyncTeleBot, message: types.Message):
    ...
    prompt = f"Translate the following text to {target_lang_code}: '{text_to_translate}'"
    translated_text = await gemini_service.generate_content_simple(api_key, prompt)

    if translated_text:
        await bot.reply_to(message, translated_text)
    else:
        await tg_helpers.send_error_reply(bot, message, f"Ошибка перевода для {user_id}", loc.get_text('translation_error_generic', lang_code))

    # Сбрасываем состояние после перевода
    user_states.pop(user_id, None)
...
async def handle_general_text_query(bot: AsyncTeleBot, message: types.Message):
    ...
    if response:
        ...
        await tg_helpers.send_error_reply(
            bot, message,
            error_log_message=f"Ошибка Gemini (текст) для {user_id}",
            user_reply_text=loc.get_text('gemini_error_response', lang_code)
        )
...
async def handle_photo_message(bot: AsyncTeleBot, message: types.Message):
    ...
    try:
        ...
        if response:
            ...
            await tg_helpers.send_error_reply(
                bot, message,
                f"Ошибка Gemini (фото) для {user_id}",
                loc.get_text('gemini_error_response', lang_code)
            )

    except Exception as e:
        await tg_helpers.send_error_reply(bot, message, f"Ошибка обработки фото для {user_id}: {e}")
